...
// DefaultRules returns the full built-in detection ruleset (base + advanced).
// func DefaultRules() []Rule {
// 	base := baseRules()
// 	return append(base, AdvancedRules()...)
// }
...
func baseRules() []Rule {
	return []Rule{
		...
		newRule("API004", "Registry Persistence", "Persistence", High,
			...
			`(?i)(RegSetValueEx\s*$$|RegCreateKeyEx\s*$$|RegOpenKeyEx\s*$$|HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Run)`,
			"regsetvalueex", "regcreatekeyex", "regopenkeyex", "currentversion\\run"),
		...
		newRule("EVA005", "Sleep Obfuscation", "Evasion", Medium,
			"Obfuscating code during sleep execution",
			`(?i)(Sleep\s*$$|time\.sleep\s*$$|WaitForSingleObject\s*$$|VirtualProtect\s*$$\s*.*\s*,\s*PAGE\_READWRITE)`,
			"sleep", "waitforsingleobject", "virtualprotect", "page_readwrite"),

		// ─── CRYPTO / ENCRYPTION ───────────────────────────────────────────────────
		newRule("CRY001", "Custom Encryption", "Obfuscation", Medium,
			"Custom encryption algorithms in malware",
			`(?i)(XOR\s*$$|AES\s*$$|RC4\s*$$|DES\s*$$|encrypt\s*$$|decrypt\s*$$)`,
			"xor", "aes", "rc4", "des", "encrypt", "decrypt"),
